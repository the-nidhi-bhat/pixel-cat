...
export default function PixelCatLanding() {
  ...
  return <main className="landing-shell">
    ...
    <section id="download" className="download-section"><div><p className="landing-kicker">Bring home your companion</p><h2>Your desktop<br /><em>deserves</em> a cat.</h2><p>Free while Pixel Cat is in its early days. No account, no cloud, no tracking.</p></div><div className="download-card"><div className="download-card-top"><span className="download-icon"><Volume2 /></span><div><strong>Pixel Cat v0.1</strong><span>Early access build</span></div></div><a className="download-button" href="https://github.com/the-nidhi-bhat/pixel-cat-site/releases/latest" target="_blank" rel="noreferrer" onClick={() => setDownloaded(true)}>{downloaded ? <><Check /> Download page opened</> : <>Download for macOS <ArrowDownToLine /></>}</a><a className="windows-button" href="https://github.com/the-nidhi-bhat/pixel-cat-site/releases/latest" target="_blank" rel="noreferrer" onClick={() => setDownloaded(true)}>Download for Windows <ArrowDownToLine /></a><small><Keyboard /> Placeholder release link · swap when signed builds ship</small></div></section>
