...
export default function PixelCatLanding() {
  ...
  return <main className="landing-shell">
    ...
    <section id="download" className="download-section"><div><p className="landing-kicker">Bring home your companion</p><h2>Your desktop<br /><em>deserves</em> a cat.</h2><p>Free while Pixel Cat is in its early days. No account, no cloud, no tracking.</p></div><div className="download-card"><div className="download-card-top"><span className="download-icon"><Volume2 /></span><div><strong>Pixel Cat v0.1</strong><span>Early access build</span></div></div><button className="download-button" onClick={() => setDownloaded(true)}>{downloaded ? <><Check /> Download started</> : <>Download for macOS <ArrowDownToLine /></>}</button><button className="windows-button" onClick={() => setDownloaded(true)}>Download for Windows <ArrowDownToLine /></button><small><Keyboard /> Works quietly in the background · local settings only</small></div></section>
